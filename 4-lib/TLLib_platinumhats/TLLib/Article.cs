    ...
    public class Article
    {
        ...
        )
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@IconName", string.IsNullOrEmpty(IconName) ? dbNULL : (object)IconName);
                ...
                cmd.Parameters.AddWithValue("@Title2", string.IsNullOrEmpty(Title2) ? dbNULL : (object)Title2);
                cmd.Parameters.AddWithValue("@Priority", string.IsNullOrEmpty(Priority) ? dbNULL : (object)Priority);

                SqlParameter imageNameParam = new SqlParameter("@OutImageName", null);
                SqlParameter articleIDParam = new SqlParameter("@OutArticleID", null);
                SqlParameter errorCodeParam = new SqlParameter("@ErrorCode", null);
                imageNameParam.Size = 100;
                errorCodeParam.Size = articleIDParam.Size = 4;
                errorCodeParam.Direction = imageNameParam.Direction = articleIDParam.Direction = ParameterDirection.Output;
                ...
                m_ArticleID = articleIDParam.Value.ToString();
        ...
        )
        {
            ...
            {
                var scon = new SqlConnection(connectionString);
                var cmd = new SqlCommand("usp_ArticleIcon_Delete", scon);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@ArticleID", string.IsNullOrEmpty(ArticleID) ? dbNULL : (object)ArticleID);
                SqlParameter errorCodeParam = new SqlParameter("@ErrorCode", null);
                errorCodeParam.Size = 4;
                ...
                if (errorCodeParam.Value.ToString() != "0")
                    throw new Exception("Stored Procedure 'usp_ArticleIcon_Delete' reported the ErrorCode : " + errorCodeParam.Value.ToString());
