...
public partial class ad_single_article : System.Web.UI.Page
{
    ...
    void DeleteIcon(string strIconName)
    {
        if (!string.IsNullOrEmpty(strIconName))
        {
            var strIconPath = Server.MapPath("~/res/article/icon/" + strIconName);
    ...
    protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "PerformInsert" || e.CommandName == "Update")
        {
            ...
            string strArticleCategoryID = "9";//((RadComboBox)row.FindControl("ddlCategory")).SelectedValue;
            ...
            if (e.CommandName == "PerformInsert")
            {
                ...
                if (!string.IsNullOrEmpty(strIconName))
                {
                    strIconName = (string.IsNullOrEmpty(strConvertedArticleTitle) ? "" : strConvertedArticleTitle + "-") +
                                  strArticleID + strIconName.Substring(strIconName.LastIndexOf('.'));
                }
